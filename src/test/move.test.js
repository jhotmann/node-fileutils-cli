...
describe('Force multiple files to be renamed the same: rename test/th* test/same --no-index --force', () => {
  const oldFiles = ['test/three.txt', 'test/thirteen.txt'];
  const newFiles = ['test/same.txt'];
  beforeAll(async () => {
    await runCommand('rename test/th* test/same --no-index --force');
  });
  test(`Old files don't exist`, async () => {
    const result = await async.every(oldFiles, async (f) => { return await fs.pathExists(path.resolve(f)); });
    await expect(result).toBe(false);
...
describe('Multiple files to be renamed the same but with keep option: rename test/six* test/keep --no-index -k', () => {
  const oldFiles = ['test/six.txt', 'test/sixteen.txt'];
  const newFiles = ['test/keep.txt', 'test/keep-1.txt'];
  beforeAll(async () => {
    await runCommand('rename test/six* test/keep --no-index -k');
  });
  test(`Old files don't exist`, async () => {
    const result = await async.every(oldFiles, async (f) => { return await fs.pathExists(path.resolve(f)); });
    await expect(result).toBe(false);
...
describe(`Rename a mp3 file: rename test/music.mp3 --create-dirs "test/{{id3.year}}/{{id3.artist}}/{{id3.track|padNumber(2)}} - {{id3.title}}.{{ext}}"`, () => {
  const oldFiles = ['test/music.mp3'];
  const newFiles = ['test/2019/Scott Holmes/04 - Upbeat Party.mp3'];
  beforeAll(async () => {
    await fs.copyFile('src/test/test_files/Scott_Holmes_-_04_-_Upbeat_Party.mp3', 'test/music.mp3');
