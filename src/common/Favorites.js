...
module.exports.Favorites = class Favorites {
  ...
  async displayFavorite() {
    clear();
    console.log(`${this.selected.id}: ${this.selected.commandString}${ this.selected.alias ? ` (${this.selected.alias})` : ''}`);
    const selection = await inquirer.prompt({
      type: 'list',
      loop: false,
  ...
  async get(idOrAlias) {
    ...
    } else if (this.options.idOrAlias && this.options.idOrAlias.match(/^\d+$/)) {
      const id = parseInt(this.options.idOrAlias);
      await this.getById(id);
    } else if (this.options.idOrAlias) {
      await this.getByAlias(this.options.idOrAlias);
    } else {
      await this.display();
    }
  }
  ...
  async run() {
    ...
    if (this.options.verbose) console.log(this.selected.commandString);
    await (fu(this.selected.command));
  }
  ...
  async editCommand() {
    ...
    const input = await term.inputField({ cancelable: true, default: this.selected.commandString, keyBindings: keyBindingOptions }).promise;
    if (input) {
      this.selected.command = input.split(' ');
